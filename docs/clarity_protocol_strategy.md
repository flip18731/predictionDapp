...
- **Solution**: Clarity Protocol – a hybrid, AI-assisted oracle that delivers citeable, on-chain evidence in ~20 seconds via Chainlink Functions and a purpose-built LLM evidence agent.
- **MVP scope**: One Solidity oracle contract + one Chainlink Functions script + a minimal Next.js dApp, all running on BNB Chain testnet, optimized for a 4-day sprint.
...
| --- | --- | --- |
| Latency | 48–96h dispute windows | AI + Chainlink Functions round trip in ~20s |
...
2. **AI as Core Utility** – KI collects sources, generates structured JSON verdicts (Supported/Refuted/Unclear) with URLs and quote snippets.
3. **Hybrid Architecture** – Solidity contract ↔ Chainlink Functions ↔ Perplexity/Gemini evidence agent.
4. **Optimistic Finality** – single trusted proposer (the AI) for MVP; disputes are deferred to post-hackathon roadmap but referenced as a next step.
5. **Sponsor Narrative** – Live demo question uses CZ's own tweet, proving we solved the sponsor's problem.

## 3. MVP Architecture
### 3.1 On-Chain: `ClarityOracle.sol`
- Imports `ChainlinkClient` to emit `requestResolution(string marketQuestion)` transactions.
- Stores `requestId → Resolution` structs (answer, confidence, summary, source array, timestamp, resolver address).
- `fulfillResolution(bytes32 requestId, bytes memory resolutionData)` gated by the authorized DON oracle.

### 3.2 Chainlink Functions Job
- JS handler receives `{ question, contextMeta }` from contract.
- Calls Perplexity (or Gemini) via HTTPS with a tightly scoped prompt demanding JSON `{ verdict, summary, sources[] }`.
- Validates schema, truncates payload to stay within gas limits, and returns ABI-encoded bytes.

### 3.3 AI Evidence Engine
- Prompt template ensures: deterministic tone, citeable URLs, quote snippets, and "confidence" scores.
- Fallback provider support (e.g., Gemini with Grounding) in case Perplexity rate limits.
...
3. **Resolution journey (60s)** – walk through contract tx → Chainlink Functions → AI call (spinner on UI, show logs in console tab if needed).
...
- ✅ Hardhat workspace scaffolded in `smart-contract/` with `ClarityOracle.sol`, tests, and deployment script.
- ✅ Chainlink Functions source drafted at `chainlink/functions/clarityEvidence.js` (Perplexity prompt + ABI encoding).
- ✅ Install dependencies (`npm install` inside `smart-contract/`). **Fix**: forcing the public npm registry + shorter fetch timeout (`npm config set registry https://registry.npmjs.org && npm install --fetch-timeout=10000 --fetch-retries=1`) surfaced progress and completed after ~4 minutes.
- ☐ Configure `.env` values (RPC, private key, Chainlink subscription, DON ID, router, gas limit).
  - _Status_: `.env` + `.env.local` skeletons are now part of the repo—replace the placeholders following the README quickstart before deploying.
- ☐ Deploy to BNB Chain testnet + record address + BscScan link here.

1. **Decentralized Proposers** – federation of AI agents (Gemini, Perplexity, Claude) competing on speed/accuracy.
...
- ✅ Chainlink Functions job ID & source file in repo (`bridge/clarity-functions.js`).
- ✅ Frontend repo (Next.js) with env template and deploy instructions.
- ✅ README quickstart + architecture diagram snippet.
- ✅ 2-minute Loom demo linked in DoraHacks submission.
...
| Optimistic finality | Single AI proposer via Chainlink Functions + configurable dispute latency (future) |
| Hybrid architecture | On-chain contract + off-chain Functions + AI APIs |
| AI-assisted proposing & evidence | Perplexity/Gemini integration returns verdict + citations |
